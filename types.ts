...
// ─── Build states (exact spec values) ────────────────────────────────────────
export type BuildState =
  | 'queued'               // waiting to start
  | 'analyzing'            // analyze_prompt tool
  | 'planning'             // create_blueprint tool
  | 'clarification_needed' // request_clarification tool — pipeline paused
  | 'generating_backend'   // generate_backend_plan tool
  | 'generating_database'  // generate_database_plan tool
  | 'generating_frontend'  // generate_frontend_plan tool
  | 'generating_content'   // inline content pass
  | 'assembling_preview'   // assemble_artifacts tool
  | 'validating'           // validate_bundle tool
  | 'complete'             // terminal success
  | 'failed';              // terminal failure

// Valid state transitions — used by state machine guard
...
export function canTransition(from: BuildState, to: BuildState): boolean {
  if (to === 'failed') return true; // always allowed
...
export interface StreamEvent {
  ...
  tool_name?: string;
  tool_call_id?: string;
  attempt?: number;        // for retry_attempt events
}

// ─── Tool names (Gemini function declarations) ────────────────────────────────
...
export interface DataModel {
  ...
  fields: DbField[];
  relations?: string[];
  indexes?: string[];      // e.g. "email (unique)"
}
...
export interface PipelineStep {
  ...
  description: string;
  toolName?: ToolName;
  progressAt: number;   // expected progress % when this step starts
}
